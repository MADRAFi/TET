...
//        ]
    ...
    private var countryList: [Country] = Country.all
    
    var body: some View {
        NavigationView {
            List(countryList, id: \.code) { country in
//                NavigationLink(destination: CountryView(country: country, cm: CountryModel(code: country.code))) {
                NavigationLink(destination: CountryView(country: country, currentTrack: TrackModel(file: country.file))) {
                    HStack {
                        Image(country.code)
                            .resizable()
